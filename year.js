...
const currentYearValueElement = document.querySelector('#currentYearValue');
...
// --- Function to fetch and render movie data for a specific year ---
function callYearMoviesAPI(year, page = 1) {
    if (!year) {
        console.error("Year is missing.");
        contentPhim.innerHTML = `<div class="col-12 text-center text-light">Không tìm thấy phim nào cho năm này.</div>`;
        paginationElement.innerHTML = '';
        return;
    }

    fetch(`${API_MOVIES_BY_YEAR}${year}?page=${page}`)
        ...
        .then(function(results) {
            ...
            renderMoviesAndPagination(paginationData, itemsData, 'year', year);

            if (results.data && results.data.titlePage) {
                currentYearValueElement.textContent = results.data.titlePage;
            } else {
                currentYearValueElement.textContent = year;
            }
        })
        .catch(function(error) {
            console.error("Error fetching year movie data:", error);
            contentPhim.innerHTML = `<div class="col-12 text-center text-light">Không tìm thấy phim nào cho năm này.</div>`;
            paginationElement.innerHTML = '';
        });
}

// --- renderMoviesAndPagination (copy from genre.js/country.js, adapt paginationBaseUrl) ---
function renderMoviesAndPagination(pagination, items, filterType, filterValue) {
    ...
    if (filterType === 'year') { // Only for year page
...
// --- Initial Calls for year.html ---
document.addEventListener('DOMContentLoaded', () => {
    currentYear = getUrlParameter('year');
    const page = getUrlParameter('page') || 1;
    callYearMoviesAPI(currentYear, page);
